...
-- Copiado do ChoiceCalculus/Choice!!
...
instance Show a => Show (V a) where -- {{{
    show (Obj x) = "Obj ~\n" ++ (indent $ show x)
    show (Dim name choices body) =
        let decl = "Dim " ++ name ++ "<" ++ (intercalate ", " choices) ++ "> in\n"
        in decl ++ show body
    show (Chc name bodies) =
        let bshow = intercalate "\n" (map (indent . show) bodies)
        in "Chc " ++ name ++ " <\n" ++ bshow ++ "\n>"
-- }}}
...
cat (LVariation v) ys = LVariation $ fmap (`cat` ys) v

-- Functor, Applicative and Monad instances for List a {{{
instance Functor List where
    fmap _ Empty = Empty
    fmap f (Cons x xs) = Cons (f x) (fmap f xs)
    fmap f (LVariation v) = LVariation (fmap (fmap f) v) -- magic
...
instance Applicative List where
    ...
    (LVariation v) <*> ls = LVariation $ fmap (<*> ls) v
...
instance Monad List where
    ...
    (Cons x xs) >>= f =
        (f x) `cat` (xs >>= f)
    (LVariation v) >>= f =
        LVariation $ fmap (>>= f) v -- demorei um tempo pra chegar nisso! 
...
nth n (LVariation v) = nth n =<< v

fold _ x0 Empty = Obj x0
fold f x0 (Cons x xs) = (x `f`) <$> (fold f x0 xs)
fold f x0 (LVariation v) = fold f x0 =<< v

-- len Empty = Obj 0
-- len (Cons _ xs) = (1 +) <$> (len xs)
-- len (LVariation v) = v >>= len
len = fold (const (+1)) 0

zipL :: List a -> List b -> List (a, b)
zipL Empty _ = Empty
zipL _ Empty = Empty
zipL (LVariation v) ys = LVariation $ fmap (`zipL` ys) v
zipL xs (LVariation v) = LVariation $ fmap (xs `zipL`) v
...
instance Selectable a => Selectable (List a) where
    ...
    select d i (LVariation v) = LVariation $ select d i v
...
-- impl1 = Chc "Impl" [
--             Obj $ (LVariation v1) <> (list " + ") <> (LVariation v1),
--             Obj $ (list "2 * ") <> (LVariation v1)
--         ]
-- ex1 = Dim "Par" ["first", "second"]
--     $ Dim "Impl" ["plus", "times"]
--     $ Obj $ (list "twice ") <> (LVariation v1) <> (list " = ") <> (LVariation impl1)

-- ex3 = Dim "A" ["a0", "a1"]
--     $ Obj $ Cons 1 (LVariation $ Chc "A" [Obj $ list [2], Obj $ list [3]])
-- -- }}}
